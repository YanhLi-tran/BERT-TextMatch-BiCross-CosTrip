...
| Exp-3 | BiEncoder | Mean | TripletLoss (Online Hard) | 0.5 | ✅ 完成 |
| Exp-4 | CrossEncoder | Mean | CosineEmbeddingLoss | 0.0 | ✅ 完成 |
| Exp-5 | CrossEncoder | CLS | CrossEntropyLoss（分类头） | — | ✅ 完成 |
| Exp-6 | CrossEncoder | Mean | CrossEntropyLoss（分类头） | — | ✅ 完成 |

### 3.2 完整指标对比

| 指标 | Bi+CLS+Cosine | Bi+Mean+Cosine | Bi+Mean+Triplet(Online) | Cross+Mean+Cosine | Cross+CLS+Classify | Cross+Mean+Classify |
|------|:------------:|:--------------:|:---------------------:|:----------------:|:-----------------:|:------------------:|
| Test Acc | 86.47% | 86.91% | **88.38%** | 87.35% | **92.34%** | 91.29% |
...
| **Exp-4 (Cross+CLS+Classify)** | **4,036** | **314** | **346** | **3,924** |
| Exp-5 (Cross+Mean+Classify) | 4,029 | 398 | 353 | 3,840 |
...
| 3 | **Online Hard Negative Mining（从零训练）** | **88.38%** | batch 内动态挑最难负样本，标准做法 |

**Online Hard Negative Mining** 在每个 batch 内，对每个正样本的 anchor，在所有负样本中挑余弦相似度最高的作为负样本。负样本随训练动态变化，模型越强对手越强，最终达到 **88.38%**，反超 Cosine 基线（87.54%）约 **0.8 个百分点**。
...
| 追求 F1 平衡（BiEncoder） | **BiEncoder + Mean + Triplet(Online)** | 88.38%，反超 Cosine（87.54%） |
...
**解决：** 用训好的 Cosine 模型做离线 Hard Negative Mining，对每个正样本对在 top-50 候选里挑相似度最高但 label=0 的句子作为难负样本，构造 30K 三元组。
