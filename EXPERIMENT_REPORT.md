...
公式在大 batch 时偏差较小（+9%），小 batch 时不可用。面试时如需估算中间值（如 30 条），可在 batch=20（46ms）和 batch=50（172ms）之间线性插值：46 + (30-20)/(50-20)*(172-46) = 88ms。但实测 batch=30 为 57ms，插值高估约 50%。建议直接引用已知实测点，避免现场估算。
