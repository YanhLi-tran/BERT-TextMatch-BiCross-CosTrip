...
**核心规律：** 总耗时 ≈ 固定开销（~10ms）+ 单对开销 × batch_size。固定开销是模型 forward 的底噪，小 batch 时占比高，大 batch 时均摊后每对成本更低。

**面试估算：** 可在实测数据点间按比例插值。例如排 30 条，介于 batch=20（46ms）和 batch=50（172ms）之间：

```
30 条 ≈ 46 + (30-20)/(50-20) × (172-46) = 46 + 0.33 × 126 ≈ **88ms**
```

**手动验算方法（从 evaluate.py 输出推算）：**

```
evaluate.py 中的 infer_speed：
  总样本: 8620 对
  总耗时: 27.32s
  → 每 batch（~64 对）耗时 = 27.32 ÷ (8620÷64) ≈ 202ms

其他 batch 大小的估算：
  batch=50: (50/64) × 202ms ≈ 158ms，实测 172ms ✅（接近）
  batch=20: (20/64) × 202ms ≈ 63ms，实测 46ms ❌（小 batch 精度变差）
```

公式在小 batch 时不准（固定开销占比高），大 batch 时接近实测。最可靠的方式是参考已知的几个实测点做插值。
