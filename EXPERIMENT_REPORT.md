...
| MCC | 0.7584 | 0.7847 | 0.8699 | 0.8503 | **0.8845** | 0.8594 |
...
| :-----: | :----: | :--------: | :--------: | :--------: | :----------: | :--------: |
| BiEncoder | 5.30 | — | — | — | 0.82s (14×) | 0.23ms |
...
> **检索速度对比：** PyTorch 暴力检索 11.82s -> FAISS 0.82s（8620 条总计），加速约 **14 倍**。
>
...
| :---------: | :------: | :--------: | :-------- |
| 1（串行） | 10.17ms | 10.17ms/对 | GPU kernel 启动开销 |
...
> 补充实测：batch=15 -> 23.48ms，batch=30 -> 57.05ms，batch=40 -> 112.64ms。

**估算公式（仅供参考）：** 从 evaluate.py 输出 27.32s / 8620 对，每 batch（约64对）约 202ms。线性推算：

- batch=50: (50/64)*202 = 158ms，实测 172ms（偏差 +9%）
- batch=20: (20/64)*202 = 63ms， 实测 46ms（偏差 -27%）

公式在大 batch 时偏差较小（+9%），小 batch 时不可用。如需估算中间值（如 30 条），可在 batch=20（46ms）和 batch=50（172ms）之间线性插值：46 + (30-20)/(50-20)*(172-46) = 88ms。但实测 batch=30 为 57ms，插值高估约 50%。建议直接引用已知实测点，避免现场估算。
...
- Mean 在 Accuracy 和 F1 上均优于 CLS（89.20% vs 87.83%）
- CLS 和 Mean 的 Recall 接近（91.69% vs 91.74%）
- 两种池化方式差距约 1.4%，Mean 略优
...
- **Recall：** 两者接近，CrossEncoder 略高（92.42% vs 91.74%）
- **AUC：** CrossEncoder 显著领先（0.9627 vs 0.9188），排序能力更强
- **速度：** batch=64 并行评估时 CrossEncoder 均摊耗时更低（3.17ms/对 vs 5.30ms/对），但单条逐对推理 BiEncoder 更快（5.30ms vs 10.17ms）

### 4.3 推理速度与检索召回（完整数据见 3.3 节）

- 速度：BiEncoder 单条查询编码 + FAISS 检索约 3ms；CrossEncoder batch 并行精排 50 条约 172ms
- 检索：Recall@1=2.49%（随机基线 0.01%），Recall@20=26.56%，Recall@50=42.24%，Recall@100=55.45%，Recall@200=67.00%
- FAISS 加速：暴力检索 11.82s -> FAISS 0.82s，加速 14 倍；单条查询 0.57ms -> 0.23ms，加速 2.5 倍
...
| 中小规模检索 | **BiEncoder + FAISS** | 0.49s 检索 8620 条候选，Recall@50=42% |
| RAG 级联方案 | **BiEncoder 粗筛 → CrossEncoder 精排** | 兼顾速度与精度 |

CrossEncoder + CLS + 分类头以 **94.22%** 测试准确率成为全部 6 组实验中的最佳方案。与前期 batch=32 的预实验相比，扩大至 64 后所有实验均有显著提升：CLS+Cosine 从 86.47% 提升至 87.83%，Mean+Cosine 从 86.91% 提升至 89.20%，CrossEncoder+CLS+Classify 从 92.34% 提升至 94.22%。

在推理速度方面，FAISS 实现了 **14 倍加速**，使 BiEncoder 的向量检索进入亚秒级，足以支撑中等规模的实时检索场景。
...
**现象：** 从零开始使用 Offline HNM 三元组训练，loss 维持在 1.0 附近不下降，Val Acc 始终约 50%。

**原因：** Offline HNM 是用训好的 87% 模型挑选的难负样本，对从零开始的模型来说这些样本跟随机采样没有区别。模型不认识任何句子，谈不上"难"。loss 不降不是因为负样本好，而是模型压根没学到东西。
...
### 6.5 Offline Hard Negative Mining 构造方法（已废弃）

以下是在阶段 2 尝试过的离线构造方案，最终因从零训练无效（6.3）、加载权重后参数不稳定崩盘（6.4）而废弃。**最终采用的 Online HNM 方案见 4.4 节。**
