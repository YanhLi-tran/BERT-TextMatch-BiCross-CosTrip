...
|:-----:|:-----------:|:------------------:|:---------------:|
| BiEncoder | 5.30 ms/对 | 0.49s (24×加速) | 0.23ms |
| CrossEncoder | 4.54 ms/对 | ❌ 不支持 | ❌ |

> CrossEncoder 的逐对推理（4.54 ms/对）快于 BiEncoder（5.30 ms/对），原因是拼接编码的 P95=48，小于 BiEncoder 两条独立编码的总 token 数（32×2=64）。但 CrossEncoder 不支持独立编码，无法做预计算和向量检索，只适合精排场景。
...
|:-----:|:-------:|:------------------:|:---------------:|:--------:|
| BiEncoder | 5.30 ms/对 | 0.49s (24×加速) | 0.23ms | 召回层（可预编码建索引） |
| CrossEncoder | 4.54 ms/对 | ❌ 不支持独立编码 | ❌ | 精排层（拼接交叉编码） |
