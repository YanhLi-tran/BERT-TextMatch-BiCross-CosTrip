...
| 实验编号 | 编码器 | 池化 | 损失函数 | margin | Epoch | 状态 |
|:-------:|:------:|:----:|:--------:|:-----:|:----:|:----:|
| Exp-1 | BiEncoder | CLS | CosineEmbeddingLoss | 0.0 | 18 | ✅ 完成 |
| Exp-2 | BiEncoder | Mean | CosineEmbeddingLoss | 0.0 | 21 | ✅ 完成 |
| Exp-3 | BiEncoder | Mean | TripletLoss (Online Hard) | 0.5 | 59 | ✅ 完成 |
| Exp-4 | CrossEncoder | Mean | CosineEmbeddingLoss | 0.0 | 18 | ✅ 完成 |
| Exp-5 | CrossEncoder | CLS | CrossEntropyLoss（分类头） | — | 30 | ✅ 完成 |
| Exp-6 | CrossEncoder | Mean | CrossEntropyLoss（分类头） | — | 29 | ✅ 完成 |

### 3.2 完整指标对比

| 指标 | Bi+CLS+Cosine | Bi+Mean+Cosine | Bi+Mean+Triplet(Online) | Cross+Mean+Cosine | Cross+CLS+Classify | Cross+Mean+Classify |
|:----|:------------:|:--------------:|:---------------------:|:----------------:|:-----------------:|:------------------:|
| 训练 Epoch | 18 | 21 | 59 | 18 | 30 | 29 |
| Test Acc | 86.47% | 86.91% | **91.24%** | 87.35% | **92.34%** | 91.29% |
| Precision | 83.46% | 84.51% | - | 87.96% | **92.78%** | 91.01% |
| Recall | **91.13%** | 90.92% | - | 87.04% | 92.10% | **91.94%** |
| F1 | 87.12% | 87.60% | - | 87.50% | **92.44%** | 91.47% |
| AUC-ROC | 0.9122 | 0.9036 | - | 0.9313 | **0.9736** | 0.9686 |
| MCC | 0.7325 | - | - | 0.7471 | **0.8469** | 0.8257 |
| 最优阈值 | 0.35 | 0.60 | 0.60 | 0.60 | argmax | argmax |

> CrossEncoder + CLS + 分类头（CrossEntropyLoss）以 **92.34%** 测试准确率全面领先其他方案。Mean + 分类头以 **91.29%** 紧随其后，CLS 在 CrossEncoder 分类模式下优于 Mean 约 1 个百分点。
...
| Exp-5 (Cross+CLS+Classify) | **4,036** | **314** | **346** | **3,924** |
...
| 3 | **Online Hard Negative Mining（从零训练）** | **91.24%**（epoch 59） | batch 内动态挑最难负样本，标准做法 |

**Online Hard Negative Mining** 在每个 batch 内，对每个正样本的 anchor，在所有负样本中挑余弦相似度最高的作为负样本。负样本随训练动态变化，模型越强对手越强，最终达到 **88.38%**，反超 Cosine 基线（86.91%）约 **1.5 个百分点**。
...
| 场景 | 推荐方案 | 理由 |
|------|---------|------|
| 追求最高精度 | **CrossEncoder + CLS + 分类头** | Test Acc 92.34%，F1 92.44%，AUC 0.9736 |
| 追求 F1 平衡（BiEncoder） | **BiEncoder + Mean + Triplet(Online)** | 91.24%，反超 Cosine 约 4 个点 |
| 追求精准、少误报 | **CrossEncoder + CLS + 分类头** | Precision 92.78%，FP 仅 314 |
| 衡量排序质量 | **CrossEncoder + CLS + 分类头** | AUC 0.9736 |
| 中小规模检索 | **BiEncoder + FAISS** | 0.49s 检索 8620 条候选，Recall@50=43% |
| RAG 级联方案 | **BiEncoder 粗筛 → CrossEncoder 精排** | 兼顾速度与精度 |

总体来看，CrossEncoder + CLS + 分类头（CrossEntropyLoss）以 **92.34%** 测试准确率成为当前最佳方案，较 BiEncoder 方案高出约 **5.4 个百分点**，验证了交互式编码器在文本匹配任务上的显著优势。

BiEncoder 和 CrossEncoder（余弦模式）在 F1 上差距极小（0.1%），两者都是有效的文本匹配方案。CrossEncoder 在排序能力（AUC）上有明显优势，适合作为 reranker；BiEncoder 在召回率和实现简洁性上更优，适合作为 base retriever。
...
- [x] Exp-1: BiEncoder + CLS + Cosine
- [x] Exp-2: BiEncoder + Mean + Cosine
- [x] Exp-3: BiEncoder + Mean + Triplet (Online Hard Negative) — 88.38%
- [x] Exp-4: CrossEncoder + Mean + Cosine
- [x] Exp-5: CrossEncoder + CLS + 分类头 — 92.34%
- [x] Exp-6: CrossEncoder + Mean + 分类头 — 91.29%
- [ ] Exp-7: 两阶段训练（Cosine → Triplet + Online Hard，margin=0.5, lr=5e-6）
- [ ] Exp-8: 最终实验数据整理与 GitHub 同步
