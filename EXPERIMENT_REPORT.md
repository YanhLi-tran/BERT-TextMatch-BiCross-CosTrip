...
| 编码器 | 逐对推理速度 | 预编码+检索(8620条) | 单条检索(FAISS) |
|:-----:|:-----------:|:------------------:|:---------------:|
| BiEncoder | 15.3 ms/对 | 0.49s (24×加速) | 0.23ms |
| CrossEncoder | 11.46 ms/对 | ❌ 不支持 | ❌ |

> CrossEncoder 的逐对推理反而快过 BiEncoder，原因是拼接编码的 P95=48，小于 BiEncoder 两条独立编码的总 token 数（32×2=64）。但 CrossEncoder 不支持独立编码，无法做预计算和向量检索，只适合精排场景。
...
|:-----:|:-------:|:---------:|:-----:|:--:|:---:|:----:|
| **BiEncoder** | 89.20% | 87.60% | **91.74%** | 89.62% | 0.9188 | 15.3 ms |
| **CrossEncoder** | **92.52%** | **92.83%** | 92.42% | **92.62%** | **0.9627** | **11.46 ms** |
...
| 编码器 | 逐对推理 | 预编码+检索(8620条) | 单条查询(FAISS) | 适用场景 |
|:-----:|:-------:|:------------------:|:---------------:|:--------:|
| BiEncoder | 15.3 ms/对 | 0.49s (24×加速) | 0.23ms | 召回层（可预编码建索引） |
| CrossEncoder | 11.46 ms/对 | ❌ 不支持独立编码 | ❌ | 精排层（拼接交叉编码） |
...
> 注：检索数据来自 BiEncoder + Mean + Cosine 模型（batch=32），batch=64 重训后模型准确率从 86.91% 提升至 89.20%，召回指标只会更高不会更低。
